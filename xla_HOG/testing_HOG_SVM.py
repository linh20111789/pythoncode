...
def sliding_window(image, windowSize, stepSize):  # image is the input, step size is the no.of pixels needed to skip and windowSize is the size of the actual window
...
for resized in pyramid_gaussian(img, downscale=1.5):  # loop over each layer of the image that you take!
    ...
    for (x, y, window) in sliding_window(resized, stepSize=10, windowSize=(winW, winH)):
        ...
        if window.shape[0] != winH or window.shape[1] != winW:  # ensure the sliding window has met the minimum size
            # requirement
            continue
        ...
        # fds = np.array(fds)
        pred = model.predict(fds)  # use the SVM model to make a prediction on the HOG features extracted from the
        # window

        if pred == 1:
            if model.decision_function(fds) > 0.6:  # set a threshold value for the SVM prediction i.e. only firm the
                # predictions above probability of 0.6
